...
az vm extension set \
  ...
  --resource-group RG-k8s-cluster \
  --settings '{"commandToExecute":"apt-get -y update && apt-get -y install nginx && apt-get -y install ansible && echo k8s: admin-server > /var/www/html/index.html && mkdir /home/init-cluster && chmod 0755 /home/init-cluster && git clone https://github.com/aleks-golubyatnikov/k8s-cluster.git /home/init-cluster"}'